...
def courbe(pourcent_chercher2,tiker_live,time1,time_name1,pourcent_chercher,pourcent_perdu,note,debugage,dejatoucher2,mirande3,mirande2,J,I,moyenne_tete,moins50p,local_min,local_max,A,B,C,D,E,F,G,df,liveprice,tout_savoir, jaune):
    global argument2
    global argument3
    ...
    try:
        ...
        plt.text(local_min[-1], F, f"F  {round(F, 5)}", ha='left', style='normal', size=10.5,
                 color='red', wrap=True)
        plt.text(liveprice, G, f"G  {round(G, 5)}", ha='left', style='normal', size=10.5,
                 color='red', wrap=True)
        ...
        if passed2 == True:
            ...
            if passed3 == True:
                ...
                if E > C:
                    ...
                    mirande2['c'].values[3] = mirande2['c'].values[2] + ((suite * (df['c'].iloc[-1] - local_max[-1])))
                if E < C:
                    mirande2['c'].values[0] = mirande2['c'].values[1] + ((suite * (local_max[-2] - local_max[-3])))
                    mirande2['c'].values[3] = mirande2['c'].values[2] - ((suite * (df['c'].iloc[-1] - local_max[-1])))
                if E == C:
                ...
                BJ = [df['c'].iloc[-1], mirande2['c'].iloc[3]]
                # --- premiere droite cotée droit ---#
                ...
                for pop in range(0, len(test_min)):
                    if test_min[pop] > local_max[-3] and test_min[pop] < df['c'].iloc[-1]:
                        verif = verif + 1
                pop = 0
                for pop in range(0, len(test_max)):
                    if test_max[pop] > local_max[-3] and test_max[pop] < df['c'].iloc[-1]:
                        verif = verif + 1
                # ----- verification qu'il n'y est pas de point mort dans la figure -----# ------------------- VERIFIER !!
                ...
                if (C - B) < (C - D) and (C - B) < (E - D) and (E - F) < (E - D) and (E - F) < (
                    ...
                    if I[1] > B and J[
                        ...
                        dejatoucher = False
                        for i in range(int(round(J[0])), df['c'].iloc[-1]):
                        ...
                        if pourcent_chercher2 >= 0.6:
                            thread = Process(target=courbe, args=(pourcent_chercher2,tiker_live,time1,time_name1,pourcent_chercher,pourcent_perdu,note,debugage,dejatoucher2,mirande3,mirande2,J,I,moyenne_tete,moins50p,local_min,local_max,A,B,C,D,E,F,G,df,df['c'].iloc[-1],tout_savoir,df['c'].values[-2]))
                            thread.start()
